...
/** This module implements range proofs for the Confidential Assets protocol.
 * 
 * The implementation differs from the Confidential Assets paper
 * (https://blockstream.com/bitcoin17-final41.pdf) and instead adapts
 * the Borromean ring signature construction into a range proof using
 * value bit decomposition.
 * (https://nt4tn.net/papers/borromean_draft_0.01_34241bb.pdf?ref=glossary.blockstream.com)
 * 
 * The main difference from the paper is an unimplemented optimization that
 * saves one scalar per ring. This optimization complicates the protocol and
 * security analysis, as it requires differentiating cases where the i-th bit
 * v_i = 0 versus otherwise, and makes calculating response points R_i less
 * straightforward. The implemented version uses Borromean ring signatures as
 * a simpler subroutine.
 * 
 * The last ring's commitment is omitted from the proof and recovered by the
 * verifier by subtracting all other digit commitments from the total, reducing
 * proof size by one group element.
 * 
 * The range proof is also used to recover the commitment's value and blinding factor
 * for the transaction receiver, which is not described in the paper. Randomness is
 * generated by seeding HMAC-DRBG with the shared ECDH key, allowing 
 * the receiver to rewind the proof using the same random values
 * the sender used. Additionally, up to 3968 bytes of message data can be
 * embedded and recovered within maximally-sized proofs.
 */
